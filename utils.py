...
# oversample 1
    ...
    # Yiqun: modify sampling for CIFAR10 variant
    elif dataset == 'CIFAR10_balanced_sample':
        channel = 3
        im_size = (32, 32)
        num_classes = 10
        mean = [0.4914, 0.4822, 0.4465]
    ...
    elif dataset == 'CIFAR10_odd_oversample': # oversample cars, cat, cat, dog, horse, truck, 
        channel = 3
        im_size = (32, 32)
        num_classes = 10
        mean = [0.4914, 0.4822, 0.4465]
        ...
        all_label_train_vec = [x[1] for x in dst_train]
        # oversample odd numbers
        weight_vec_2 = np.array([90 if x==0 else 1 for x in all_label_train_vec])
        weight_vec_2 = weight_vec_2/np.sum(weight_vec_2)
        imbalanced_index_2 = np.random.choice(len(all_label_train_vec), size=subset_size, 
                                  replace=False, p=weight_vec_2)
        dst_train = torch.utils.data.Subset(dst_train, imbalanced_index_2)
